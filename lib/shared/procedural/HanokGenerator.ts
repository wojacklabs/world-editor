...
function pushChogaRoof(
  ...
  const exposedStep = 0.86 / layerRows;
  const overlapScale = 2.55;
  for (let side = -1; side <= 1; side += 2) {
    for (let row = 0; row < layerRows; row++) {
      ...
      const courseLen = clamp(
        roofLength * (0.985 - row * 0.002 + seeded01(rowSeed + 3.6) * 0.007),
        roofLength * 0.84,
        roofLength * 0.998
      );
      ...
      // Build the lower mass with medium chunks instead of one long strip.
      const underChunkCount = Math.max(3, Math.round(courseLen / 3.1));
      const underChunkSpan = courseLen / underChunkCount;
      for (let chunk = 0; chunk < underChunkCount; chunk++) {
        const underSeed = rowSeed + chunk * 17.3 + 37.0;
        const underLen = underChunkSpan * (1.04 + seeded01(underSeed + 1.2) * 0.28);
        const underX =
          -courseLen * 0.5 + (chunk + 0.5) * underChunkSpan + (seeded01(underSeed + 2.1) - 0.5) * underChunkSpan * 0.20;
        const underRoofX = underX * (roofLength / Math.max(0.001, courseLen));
        const underXNorm = clamp((underRoofX + halfRoofLength) / roofLength, 0, 1);
        const underThickness = rowThickness * (0.54 + seeded01(underSeed + 3.8) * 0.18);
        const underDepth = rowDepth * (0.84 + seeded01(underSeed + 4.9) * 0.18);
        const underY = rowY - rowThickness * (0.08 + seeded01(underSeed + 5.6) * 0.04);
        const underZ = rowZ + side * rowDepth * (0.02 + seeded01(underSeed + 6.4) * 0.03);
        geometries.push(
          createStrawBundle(
            underLen,
            underThickness,
            underDepth,
            new THREE.Vector3(underX, underY, underZ),
            makeWeatheredStrawColor(underSeed + 7.2, tInner, underXNorm, side, 0.10),
            new THREE.Euler(
              slopeAngle + (seeded01(underSeed + 8.1) - 0.5) * 0.008,
              (seeded01(underSeed + 9.0) - 0.5) * 0.004,
              (seeded01(underSeed + 9.9) - 0.5) * 0.003
            ),
            12
          )
        );
      ...
      const segmentCount = Math.max(11, Math.round(courseLen / 0.74));
      const segmentSpan = courseLen / segmentCount;
      for (let i = 0; i < segmentCount; i++) {
        const segSeed = rowSeed + i * 19.7 + 70.0;
        const segLen = segmentSpan * (0.76 + seeded01(segSeed + 1.7) * 0.40);
        const segX =
          -courseLen * 0.5 + (i + 0.5) * segmentSpan + (seeded01(segSeed + 2.9) - 0.5) * segmentSpan * 0.24;
        ...
        const segY = rowY + rowThickness * (0.07 + seeded01(segSeed + 5.6) * 0.08) + ridgeWarp;
        const segZ =
          rowZ - side * rowDepth * (0.10 + seeded01(segSeed + 6.7) * 0.08) + side * ridgeWarp * 0.5;
        const segThickness = rowThickness * (0.36 + seeded01(segSeed + 7.8) * 0.14);
        const segDepth = rowDepth * (0.24 + seeded01(segSeed + 8.9) * 0.12);
        geometries.push(
          createStrawBundle(
            segLen,
            segThickness,
            ...
            new THREE.Euler(
              slopeAngle + (seeded01(segSeed + 10.4) - 0.5) * 0.012,
              (seeded01(segSeed + 11.6) - 0.5) * 0.008,
              (seeded01(segSeed + 12.8) - 0.5) * 0.006
            ),
            10
          )
        );

        if (seeded01(segSeed + 14.1) > 0.74) {
          geometries.push(
            createStrawBundle(
              segLen * (0.46 + seeded01(segSeed + 14.8) * 0.18),
              segThickness * (0.42 + seeded01(segSeed + 15.7) * 0.16),
              segDepth * (0.52 + seeded01(segSeed + 16.6) * 0.16),
              new THREE.Vector3(
                segX + (seeded01(segSeed + 17.5) - 0.5) * segmentSpan * 0.14,
                segY + rowThickness * (0.10 + seeded01(segSeed + 18.4) * 0.06),
                segZ - side * rowDepth * (0.05 + seeded01(segSeed + 19.3) * 0.04)
              ),
              makeWeatheredStrawColor(segSeed + 20.1, tInner, xNorm, side, 0.10),
              new THREE.Euler(
                slopeAngle + (seeded01(segSeed + 21.2) - 0.5) * 0.014,
                (seeded01(segSeed + 22.3) - 0.5) * 0.010,
                (seeded01(segSeed + 23.4) - 0.5) * 0.008
              ),
              9
            )
          );
      ...
      if (row === 0) {
        const fringeCount = Math.max(18, Math.round(roofLength / 0.52));
        for (let i = 0; i < fringeCount; i++) {
          const fringeSeed = rowSeed + i * 17.1 + 310.0;
          const fringeW = 0.05 + seeded01(fringeSeed + 1.2) * 0.03;
          const fringeH = 0.09 + seeded01(fringeSeed + 2.4) * 0.06;
          const fringeD = 0.02 + seeded01(fringeSeed + 3.6) * 0.018;
          const fringeX =
            -halfRoofLength + ((i + 0.5) / fringeCount) * roofLength + (seeded01(fringeSeed + 4.8) - 0.5) * 0.11;
          const fringeZ = side * roofSpan * 0.5 * tExposed + side * (fringeD * 0.64 + rowDepth * 0.06);
          const fringeY = yExposed - fringeH * (0.44 + seeded01(fringeSeed + 5.9) * 0.14);
          geometries.push(
            createBox(
              fringeW,
              fringeH,
              ...
              new THREE.Euler(
                ...
                (seeded01(fringeSeed + 10.8) - 0.5) * 0.06
              )
            )
          );
        }
  ...
  // Short eave skirt bundles: adds rounded, heavy thatch lip instead of sharp plank edge.
  for (let side = -1; side <= 1; side += 2) {
    const skirtCount = Math.max(8, Math.round(roofLength / 1.35));
    for (let i = 0; i < skirtCount; i++) {
      const skirtSeed = seed + side * 151.7 + i * 23.4 + 980.0;
      const segmentSpan = roofLength / skirtCount;
      const skirtLen = segmentSpan * (0.96 + seeded01(skirtSeed + 1.8) * 0.28);
      const x = -halfRoofLength + (i + 0.5) * segmentSpan + (seeded01(skirtSeed + 2.9) - 0.5) * segmentSpan * 0.22;
      ...
      geometries.push(
        createStrawBundle(
          ...
          new THREE.Euler(
            ...
            (seeded01(skirtSeed + 11.0) - 0.5) * 0.05
          ),
          8
        )
      );
  ...
  // Rope bands that bind the thatch courses.
  const ropeTs = isMainWing ? [0.86, 0.70, 0.55] : [0.84, 0.67];
  for (let side = -1; side <= 1; side += 2) {
    for (let i = 0; i < ropeTs.length; i++) {
      const ropeSeed = seed + side * 173.3 + i * 41.2 + 1430.0;
      const t = ropeTs[i];
      const ropeY = surfaceY(t) + 0.012 + i * 0.004;
      const ropeZ = side * roofSpan * 0.5 * t;
      const ropeLen = roofLength * (0.94 - i * 0.03);
      const ropeRadius = (isMainWing ? 0.026 : 0.022) * (1 - i * 0.10);
      geometries.push(
        createCylinder(
          ropeRadius * 0.94,
          ropeRadius,
          ropeLen,
          10,
          new THREE.Vector3(0, ropeY, ropeZ),
          COLORS.chogaRope,
          new THREE.Euler(0, 0, Math.PI * 0.5)
        )
      );

      const tieCount = Math.max(4, Math.round(ropeLen / 2.45));
      for (let j = 0; j < tieCount; j++) {
        const tieX = -ropeLen * 0.5 + ((j + 0.5) / tieCount) * ropeLen;
        const tieScale = 0.013 + seeded01(ropeSeed + j * 7.9 + 2.1) * 0.010;
        geometries.push(
          createBox(
            tieScale,
            tieScale * 1.5,
            tieScale,
            new THREE.Vector3(tieX, ropeY + tieScale * 0.7, ropeZ),
            COLORS.chogaRope
          )
        );
      }
  ...
  const ridgeTuftCount = Math.max(5, Math.round(roofLength / 1.9));
...
function generateHanokLinearHouseGeometry(
  ...
  const pushChogaPanelSurfaceZ = (
    ...
    const plasterCount = Math.max(2, Math.min(8, Math.round(area * 1.15)));
    for (let i = 0; i < plasterCount; i++) {
      ...
      const smearW = panelWidth * (0.22 + seeded01(s + 3.9) * 0.34);
      const smearH = panelHeight * (0.20 + seeded01(s + 4.8) * 0.24);
      const smearD = wallThickness * (0.12 + seeded01(s + 6.1) * 0.14);
      ...
      if (seeded01(s + 8.7) > 0.74) {
        geometries.push(
          createBox(
            smearW * (0.52 + seeded01(s + 9.4) * 0.26),
            smearH * (0.40 + seeded01(s + 10.3) * 0.22),
            smearD * (0.42 + seeded01(s + 11.1) * 0.24),
            new THREE.Vector3(
    ...
    const crackCount = Math.max(1, Math.min(5, Math.round(area * 0.62)));
    for (let i = 0; i < crackCount; i++) {
      const s = seed + offset * 1.61 + i * 13.4;
      const crackLen = panelWidth * (0.16 + seeded01(s + 1.9) * 0.24);
      const crackH = 0.008 + seeded01(s + 2.8) * 0.012;
      const crackD = wallThickness * (0.018 + seeded01(s + 3.6) * 0.016);
  ...
  const pushChogaPanelSurfaceX = (
    ...
    for (let i = 0; i < plasterCount; i++) {
      ...
      const smearD = panelDepth * (0.22 + seeded01(s + 4.2) * 0.34);
      const smearH = panelHeight * (0.20 + seeded01(s + 5.1) * 0.24);
      const smearX = wallThickness * (0.12 + seeded01(s + 6.6) * 0.14);
      ...
      if (seeded01(s + 8.4) > 0.74) {
        geometries.push(
          createBox(
            smearX * (0.42 + seeded01(s + 9.2) * 0.26),
            smearH * (0.40 + seeded01(s + 10.4) * 0.22),
            smearD * (0.52 + seeded01(s + 11.8) * 0.24),
            new THREE.Vector3(
    ...
    for (let i = 0; i < crackCount; i++) {
      ...
      const crackD = panelDepth * (0.16 + seeded01(s + 1.5) * 0.24);
      const crackH = 0.008 + seeded01(s + 2.4) * 0.012;
      const crackX = wallThickness * (0.018 + seeded01(s + 3.7) * 0.016);
